...
`*IGA,<PID>,<IGAID>,<File>[,rr,rs,rt,ratio,ir,pr,ps,pt,nisr,niss,nist]`
...
| 9 | `pr` | NURBS 다항식 차수 (r 방향) | 선택 | `1` | 2026-08 추가 |
| 10 | `ps` | 〃 (s 방향). 생략 시 `pr` | 선택 | `pr` | 2026-08 추가 |
| 11 | `pt` | 〃 (t 방향). 생략 시 `pr` | 선택 | `pr` | 2026-08 추가 |
| 12 | `nisr` | Gauss 적분점 수 (r 방향). 1=reduced, 2=정밀 | 선택 | `1` | 2026-08 추가 |
| 13 | `niss` | 〃 (s 방향). 생략 시 `nisr` | 선택 | `nisr` | 2026-08 추가 |
| 14 | `nist` | 〃 (t 방향). 생략 시 `nisr` | 선택 | `nisr` | 2026-08 추가 |

### 차수(pr/ps/pt)와 적분점(nisr/niss/nist) — 2026-08 추가

```
*IGA,1,100,iga.k,2.0,2.0,2.0,1.1,0,2          → 전 축 2차
*IGA,1,100,iga.k,2.0,2.0,2.0,1.1,0,3,2,1      → 축별 3/2/1
*IGA,1,100,iga.k,2.0,2.0,2.0,1.1,0,2,2,2,2    → 2차 + 적분점 2 균일
```

- 제어점 개수 `nr/ns/nt` 는 `max(2, p+1)` 로 자동 산정된다. 차수만 올리고
  `nr` 을 2로 두면 제어점이 모자라 LS-DYNA 가 패치를 만들지 못한다.
- 차수가 2 이상인데 적분점이 1 이면 안내가 출력된다(가이드 권장은 2).
  경고가 아니라 정보다 — explicit 해석에서 reduced 는 의도적 선택일 수 있다.
- 차수·적분점을 생략하면 1차/적분점1 로, 출력이 예전과 **바이트 동일**하다.

> 🔴 **과거 결함 (2026-08 수정)**: 제어점 생성이 "박스 꼭짓점 8개" 로 하드코딩돼
> 있었다. `nr/ns/nt` 를 올려도 제어점이 8개 그대로라 고차 패치가 성립하지
> 않았다(예: `pr=2` → `nr=3`, 27개 필요한데 8개). 지금은 `nr×ns×nt` 격자로
> 생성하며, 중간 제어점 좌표는 `*PARAMETER_EXPRESSION_LOCAL` 로 나간다.
