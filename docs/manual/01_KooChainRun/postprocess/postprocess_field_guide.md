...
| `element_type` | `solid` · `thick_shell` · `shell` — 기준 × 요소 종류마다 항목이 따로 |
| `peak_layer` | 셸 계열: 극값이 난 적분점 층. `layer_scheme: mid_inner_outer` 면 0 중립·1 안쪽·2 바깥쪽 면 |
| `uniform` | 🔴 **true 면 평탄 분포** — 상위 값이 파트 전체에 같다. 덩어리가 있어도 위치·개수에 의미 없음 |

**셸** — 굽힘 응력은 표면에서 최대라 적분점 층 중 가장 뜨거운 층을 쓴다(중립면만 보면 놓친다).
평균·중심은 면적×두께 가중(덱에 두께 출력이 없으면 면적). 덩어리에 `area` 가 나오고,
`volume` 은 두께가 있을 때만 나온다.

**리포트** — deep report HTML 에 '핫스팟 군집' 탭이 생긴다. 순위표 → 파트 선택 → 덩어리 표와
파트 평면 그림(원 반지름 = 포함 반경). 셸 계열이 있는 모델이면 Overview 의 '피크 응력' 에
**(솔리드만)** 이 붙는다 — 응력 시간이력 집계가 솔리드 전용이기 때문이다. 셸 최대는 이 탭에서 본다.
...
|---|---|
| 솔리드 요소 핫스팟 | 지원 |
| 셸·두꺼운 셸 핫스팟 | 지원 (SIF v32~) — 적분점 층 중 가장 뜨거운 층, 면적×두께 가중 |
| 핫스팟 HTML 표시 | 지원 (SIF v32~) — deep report 의 '핫스팟 군집' 탭 |
| 핫스팟 기준량 | von Mises · σ1 · σ3 (SIF v31~) — 변형률 기준 군집은 미지원 |
| 시각별 군집 추적 | 미지원 (전 시간 최대 기준 1회) |
| 구역 라벨("좌하단") | HTML 리포트에서 지원 (SIF v32~) — 파트 평면 두 축 기준. JSON 에는 `bbox_min/max` 로 상대 좌표만 |
