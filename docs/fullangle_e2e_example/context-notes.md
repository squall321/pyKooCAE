...
(진단은 아래 `ulimit -v` 절에서 확정됐다 — 변형 차이도 비정성도 아니었다.)

## 🔴🔴 원인 확정 — `ulimit -v` 40 GB 가 후처리를 무언 축소시킨다
"두 변형 차이" 도 "간헐적 비정성" 도 아니었다. **내가 light 패스에만 걸어 둔
`ulimit -v 40000000`(40 GB)** 이 원인이다. full 패스와 재실행은 60 GB 였다.

통제 실험 (`Run_20260522_132307_0d0978`, PID 17 `peak_acc_mag`).

| 조건 | 값 | 횟수 |
|---|---|---|
| `ulimit -v` 40 GB | 186,397,893.2 | 2 (원 light 패스, 통제 재실행) |
| `ulimit -v` 60 GB | 192,770,615.4 | 3 (재실행1, 재실행2, full) |

방향성도 맞아떨어진다 — 전수 비교 불일치 **8건 전부 light(40 GB) 쪽이 작다.**
무작위라면 반반이어야 한다(단면 확률 1/256). 즉 **놓친 최대값**이다.

🔴 위험한 점.
- 경고가 **하나도** 없다. 로그가 경로만 빼고 바이트 동일하고 `1001개 타임스텝` 도 같게 찍힌다
- `rc=0` 으로 정상 종료한다
- 피크를 **낮게** 보고한다 = 안전계수를 **과대평가**한다. 구조 판정에서 가장 위험한 방향
- 20방향 중 5방향에서만 나타났다 = 40 GB 는 요구량(최대 RSS 33 GiB, VA 는 더 큼)에
  간신히 걸치는 한도라 실행마다 천장에 닿을 때만 축소된다. 재현이 안 되는 이유

조치.
- 드라이버 기본 `VMEM_KB` 를 60 GB 로 (33 GiB 작업에 40 GB 는 너무 촘촘하다)
- light 20방향 전량을 60 GB 로 재생성 → 재비교로 불일치 0 확인
- 40 GB 산출물은 `evidence_reproducibility/` 에 증거로 보존

교훈: 런어웨이 방어용 `ulimit` 을 실측 요구량 바로 위에 두면 방어가 아니라 **조용한 오답 생성기**가
된다. 실측의 1.8배 이상으로 잡는다. 그리고 "같은 입력·같은 옵션인데 값이 다르다" 를 만나면
**내가 건 실행 조건**부터 의심한다 — 도구 결함으로 단정하기 전에.
