...
> **A-1 은 상류에서 별건으로 수정 완료**(커밋 f067ab3) — 아래 §A-1 후기 참조.
...
#### 상류 처리 결과 (2026-08-28, 커밋 f067ab3)

수정했습니다. 다만 **보고서가 지목한 범위보다 넓었습니다.**

- 보고서는 `DropWeightImpactTestwithPartialRigid` 만 지목했으나, 같은 SI 폴백이
  `DropWeightImpactTest`(기본 경로 `GenerationMode,DampingSpring`) 와
  `DropWeightImpactTestbyPart` 에도 있었습니다. **세 함수 전부** 배선했습니다.
- `_warn_dwi_fallback()` 공용 헬퍼가 누락 키·실제로 쓰인 폴백값·ton-mm-s 환산 힌트를
  출력합니다.
- DWI 파서 말단 `else` 추가 — 인식 못 한 줄을 무시했다는 사실을 알립니다.

**폴백값 자체는 바꾸지 않았습니다.** ton-mm-s 로 교체하면 이 폴백에 의존해 돌던 기존
덱의 결과가 조용히 달라집니다. "조용한 실패"를 "조용한 값 변경"으로 바꾸는 것이라
고칠 지점을 알려주는 쪽이 맞다고 판단했습니다.

검증 — 정상 덱 산출물 md5 동일(`a627f33e…`, 경고만 추가·동작 불변), 실덱 8종 파서
왕복 회귀 0, 누락/오타 덱에서 경고 실출력 확인.
