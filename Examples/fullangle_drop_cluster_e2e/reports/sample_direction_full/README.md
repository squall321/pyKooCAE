...
근사 동률이 아니라 2% 차이다 = 한쪽이 그 상태를 놓쳤다는 뜻이다. 요소 단위(응력·변형률) 스캔에만
나타나고 절점 단위(변위)에는 없다. 후처리(`KooD3plotReader`) 쪽 재현성 문제로 보이며
이 예제 패키지의 범위 밖이다. 전 방향 비교 결과는 `../../RESOURCES.md` 에 싣는다.
