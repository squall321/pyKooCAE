...
class LargeScaleDOEManager:
    ...
    def submit_step_array_job(
        ...
        with open(script_path, 'w') as f:
            ...
            f.write("# ====================================================================\n")
            f.write("# KooMeshModifier용 txt 파일 작성\n")
            f.write("# ====================================================================\n")
            f.write("# 규칙: data/result 경로는 상대 경로로 작성\n")
            f.write("#   - data/result → ./\n")
            f.write("#   - 이전 Step dynain → ../Step{n-1:03d}/dynain\n")
            f.write("# ====================================================================\n")
            f.write("\n")
            f.write('INPUT_TXT="$STEP_DIR/input.txt"\n')
            f.write('cat > "$INPUT_TXT" << EOF\n')
            f.write("# KooMeshModifier Input Configuration\n")
            f.write("# Auto-generated by LargeScaleDOEManager\n")
            f.write("\n")
            ...
            # Step 1이면 이전 dynain 없음, Step 2+면 이전 Step dynain 참조
            if step_number == 1:
                f.write("# Step 1: 이전 dynain 없음\n")
            else:
                f.write("# 이전 Step dynain 경로\n")
