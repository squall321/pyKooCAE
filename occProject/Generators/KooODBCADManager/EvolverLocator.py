...
def find_linux_evolver(basePath=None):
    ...
    그 폴더를 만들고 evolver 링크를 둔다. 못 찾으면 이유를 찍고 종료 코드 1."""
    ...
    local = os.path.join(workdir, "evolver")
    if not os.path.exists(local):
        os.makedirs(workdir, exist_ok=True)
        try:
            os.symlink(os.path.abspath(found), local)
        except OSError:
            shutil.copy2(found, local)
        print(f"Evolver 작업 폴더 준비: {workdir} → {found}")
    return found
