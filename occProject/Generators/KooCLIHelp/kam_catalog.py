...
TOPICS = [
    Topic("cli", "명령 형식 — 모드 이름은 대소문자 그대로", aliases=["사용법", "인자", "argv", "usage"], body=[
        ...
        "  - 종료 코드는 성공·실패 모두 0 이다. 성패는 로그의 Complete/FAILED 와 산출물 존재로 판단한다.",
        "  - 실행 시 등록 IP·사용 기한(2027-12-31)을 확인한다. `Access denied` 가 찍히면 그 머신에서는 실행되지 않는다 (종료 코드 0).",
    ]),
...
mode("CAPACITOR", "MLCC 칩 커패시터(패드·터미널·솔더·유전체 적층) 형상과 메시 생성",
     ...
     notes=["솔더 형상은 Surface Evolver 로 계산하므로 Library/Evolver 가 필요하다."],
     related=["PKG"])
...
CATALOG = Catalog(
    ...
    overview=["모드 이름은 대소문자 그대로 (PKG, CAPACITOR/CAP, PCB, ArrayPCB, PBA, AIRMESH).",
              "종료 코드는 항상 0 — 성패는 로그와 산출물로 판단 (--help cli)."],
