...
class ElementSolid(DynaKeyword):
    ...
    def _split_header(line):
        ...
        if len(toks) == 2 and _ok(toks[0]) and _ok(toks[1]):
            # 🔴 "2토큰이면 헤더"는 위험한 판정이다. 표준 1줄 솔리드
            #    (EID PID N1..N8)에서 노드ID가 8자리라 필드를 꽉 채우면 노드끼리
            #    공백 없이 붙어, split() 이 딱 2토큰을 낸다.
            #      '41647216 2244284331362173313621833136219...'
            #       └─EID──┘ └────── PID+노드 전체가 한 덩어리 ──────┘
            #    이걸 헤더로 오인하면 PID 가 71자리 쓰레기가 되어 요소가 통째로
            #    사라진다(예외도 안 난다). 실제 M3 모델 100/100 실패 원인.
            #
            #    LS-DYNA 정수 필드는 최대 10칸(I10)이다. 그보다 긴 토큰은 병합된
            #    것이 확실하므로 헤더로 인정하지 않는다. 길이 기준이 아니라 폭
            #    기준이라, 패딩이 많은 자유간격 헤더는 그대로 통과한다.
            if len(toks[0]) <= 10 and len(toks[1]) <= 10:
